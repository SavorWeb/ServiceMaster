<?php include("inc/head.html"); ?>

<div class="wrapper">
	
	<div class="pageimg">
		<img class="editable" id="header-img" src="assets/photos/hardhats.jpg" alt="A Service Master!">
	</div>

	<div class="padded"> <!-- START CONTENT -->
		
		<section>
		<article>
			<div class="full editable" id="legal-1">
				<h1>Privacy Policy</h1>
				<img class="full-width" src="assets/photos/promo/restore-header.jpg" alt="">
				<p>
					Your privacy is important to us and we are committed to respecting it.  We maintain physical, electronic, and procedural safeguards that comply with federal regulations to guard you non-public information. In addition we regularly review our policies and practices, monitor our computer networks and test the strength of our security.
				</p>

				<p>
					The privacy policy below describes how and when ServiceMaster of North Idaho (SMNI) collects and uses your information when you use our website.
				</p>
			</div>
			<div class="half editable" id="legal-2">
				<h2>What Information Do We Collect?</h2>
				<p>
					We collection and use information we need to conduct our business, to advise you of our products and services, and to provide you with customer service. We may collect and maintain several types of information needed for these purposes, such as those below:
				</p>

				<p>
					From you: If you choose to submit a message to us by clicking the “Submit” link on our website, emails, telephone calls and in-person contact with our staff. All personally identifiable information (PII) collected with be stored and maintain on our local on-site servers.
				</p>

				<p>
					From your visit to our website: we us several common tracking technologies, these may include browser cookies, analytical tools, or other technologies. If you click on a link to a third party site, you will be taken to websites we do not control. This policy does not apply to the privacy practices of that website.
				</p>
			</div>

			<div class="half editable" id="legal-3">
				<h2>How We May Use Your Information</h2>
				<p>
					We use your PII information to process your claims, ensure proper billing, service your job, enhance your customer experience, and offer your additional services that may suit your needs.
				</p>
				<p>
					We use your electronic information to acknowledge your visit to our site, improve our site, and better understand the interests of our customers and visitors.
				</p>
				<p>
					We do not sell information. We do not share information with any unaffiliated third parties.
				</p>
				<p>
					We may share information about you with persons or organizations outside of our company for our everyday business purposes and as permitted or required by law. We may also share information with other entities as needed to handle your job, such as subcontractors.
				</p>
			</div>
			<section class="full editable" id="last">
				<h2>Instruction to Opt-Out/Modify of Future Communications</h2>
				<p>
					You may request to opt-out/modify future communications by sending an e-mail to office@smnorthidaho.com or by mailing your request to: 1007 N Boulder Court Post Falls ID 83854. Please specify the nature of your request.
				</p>
			</section>
		</article>
	</section>


</div> <!-- END MAIN CONTENT -->

<?php include("inc/foot.html"); ?>